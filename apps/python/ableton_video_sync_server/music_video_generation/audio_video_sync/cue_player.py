...
# ============================================================
# Utility: fade in/out and simple chirps
# ============================================================
...
# ============================================================
# ============================================================
# Deterministic Sound Generation (longer, musical)
# ============================================================
def unique_cue(seed: int, length=1.3, fs=FS) -> np.ndarray:
    """
    Rhythmisch und klanglich sehr unterschiedliche Mono-Cues.
    Jeder Seed generiert eine eigene 'Mini-Sequenz' aus kurzen Noten und Noise-Hits.
    """
    ...
    # Random: Anzahl und Positionen kurzer Noten
    ...
    for i in range(num_notes):
        ...
        # Kurze, leicht gefilterte Wellen (Sinus + leicht verzerrt)
        ...
        # kleine Noise-Burst-Anteile (Attack-Klicks)
        if rng.random() < 0.4:
    ...
    # sanfter Gesamt-Fade, Normalisierung
    tone = fade(tone, ms=FADE_MS * 2)
    tone /= np.max(np.abs(tone) + 1e-6)

    # leicht dumpfer oder heller Filter je Seed
    if rng.random() < 0.5:
...
# ============================================================
# Robust Sync Markers (Barker-13 BPSK)
# ============================================================
def mk_barker_bpsk(chip_ms: float = 18.0, carrier_hz: float = 3000.0, fs: int = FS) -> np.ndarray:
    """
    Generate a short BPSK burst using the Barker-13 sequence on a single carrier.
    - chip_ms: chip duration in ms (typ. 1525ms)
    - carrier_hz: carrier frequency (2.53.5 kHz recommended for webcams/mics)
...
class CuePlayer:
    ...
    def play(self, stereo: np.ndarray, samplerate=FS):
        if self.backend_name == "sounddevice":
            self.sd.play(stereo, samplerate=samplerate, blocking=True, device=self.device_index)
    ...
    # ------------- Deterministic play by musical bar -------------
    def play_bar(self, bar_index: int, total_bars: int, dur=0.5):
        """
        Derives a reproducible seed from bar context.
        Example: use bar_index + total_bars as seed space.
        """
        seed = (bar_index * 7919 + total_bars * 104729) % (2**31 - 1)
        self.play_seed(seed, dur=dur)

    # ------------- Barker-13 BPSK marker playback -------------
...
# ============================================================
# Optional manual test / key trigger (like your old script)
# ============================================================
def key_is_down(vk_code: int) -> bool:
    return bool(_user32.GetAsyncKeyState(vk_code) & 0x8000) if IS_WINDOWS else False


def main():
    from audio_output_selection import AudioOutputSelector

    print("CuePlayer Singleton Test  automatische Device-Erkennung ")
    ...
    print(f" CuePlayer verwendet Device {device_index}: {selector.devices[device_index]['name']}")

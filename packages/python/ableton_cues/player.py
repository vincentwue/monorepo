...
def fade(x: np.ndarray, ms: int = FADE_MS, fs: int = FS) -> np.ndarray:
...
def unique_cue(seed: int, length: float = 1.3, fs: int = FS) -> np.ndarray:
...
class CuePlayer:
    ...
    def play_barker(self, chip_ms: float = 18.0, carrier_hz: float = 3000.0, gain: float = 1.0, blocking: bool = True) -> None:
        def _job() -> None:
